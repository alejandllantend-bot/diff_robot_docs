...
![Cmake](imagenes_G/cmake.png)
![Robot_developer](imagenes_G/robot_dev.png)
...
Una vez completada la instalación anterior, se procede a instalar el siguiente paquete el cual incluye ejemplos y recursos para trabajar con archivos URDF, utilizados en la descripción de la estructura y componentes de un robot.
...
instala los paquetes necesarios para conectar ROS 2 con el simulador Gazebo, lo que permite la comunicación entre ambos sistemas, facilitando el envío y recepción de datos como sensores y comandos de control del robot dentro del entorno simulado.
...
Para que todos estos paquetes funcionen correctamente, es necesario ejecutar el comando source /opt/ros/jazzy/setup.bash, el cual carga las variables de entorno de ROS 2 en la terminal. Sin este paso, los comandos de ROS no estarán disponibles. Con el fin de evitar ejecutar este comando manualmente en cada sesión, se recomienda agregar la línea source /opt/ros/jazzy/setup.bash al final del archivo .bashrc, lo que permite que el entorno se configure automáticamente al abrir una nueva terminal.
